...
const Reports = () => {
  ...
  // تاريخ اليوم (ديناميكي حسب اللغة)
  ...
  // نصوص التقرير المتغيرة (يمكن نقلها لملف الترجمة لاحقاً)
  const reportTexts = {
    ...
    title: reportType === 'inventory' 
      ? (lang === 'ar' ? 'كشف جرد المخزون' : 'Inventory Report') 
    ...
    subjectBody: reportType === 'inventory'
      ? (lang === 'ar' ? 'فيما يلي بيان تفصيلي بجميع الأرصدة المتاحة بالمخازن حتى تاريخه.' : 'Below is a detailed statement of all available stock in warehouses to date.')
      : (lang === 'ar' ? 'يرجى الإحاطة بوجود عجز في الأرصدة الموضحة أدناه، ونوصي بسرعة التوريد.' : 'Please note the shortage in the items listed below; urgent supply is recommended.'),
    recTitle: lang === 'ar' ? "توصيات النظام الآلي:" : "Automated Recommendations:",
    recSafe: lang === 'ar' ? "المخزون في الحدود الآمنة." : "Stock levels are safe.",
    recDanger: lang === 'ar' ? `يوجد (${reportData.filter(i => i.stock < 50).length}) أصناف تحت حد الخطر.` : `There are (${reportData.filter(i => i.stock < 50).length}) items below safety level.`,
    footerNote: lang === 'ar' ? "تم إنشاء هذا التقرير آلياً بواسطة نظام ترياق." : "Automatically generated by Tiryaq System.",
    signStore: lang === 'ar' ? "أمين المخزن" : "Store Keeper",
    signManager: lang === 'ar' ? "مدير المستشفى" : "Hospital Manager"
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-10">
      ...
      {/* الهيدر (يختفي عند الطباعة) */}
      <div className="flex justify-between items-center print:hidden">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-2">
            {t.reports} 📄
          </h1>
          <p className="text-gray-500">{lang === 'ar' ? 'استخراج الوثائق الرسمية' : 'Extract Official Documents'}</p>
        </div>
        <button 
          onClick={handlePrint}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 print:block">
        ...
        {/* 1. لوحة التحكم (تختفي عند الطباعة) */}
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm h-fit print:hidden">
          <h3 className="font-bold text-lg text-gray-800 dark:text-white mb-4">{lang === 'ar' ? 'إعدادات التقرير' : 'Report Settings'}</h3>
          
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">{lang === 'ar' ? 'نوع التقرير' : 'Report Type'}</label>
              <select 
              ...
              >
                <option value="inventory">{lang === 'ar' ? 'جرد شامل' : 'Full Inventory'}</option>
                <option value="shortage">{lang === 'ar' ? 'النواقص' : 'Shortages'}</option>
              </select>
            </div>

            <div>
              <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">{lang === 'ar' ? 'التاريخ' : 'Date'}</label>
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-slate-700">
              <p className="text-xs text-gray-500 mb-2">{lang === 'ar' ? `عدد السجلات: ${reportData.length}` : `Records count: ${reportData.length}`}</p>
          ...
          <div className="bg-white text-black p-12 rounded-xl shadow-2xl min-h-[1123px] w-full max-w-[794px] mx-auto relative print:shadow-none print:m-0 print:w-full print:p-0" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <div className="mb-8">
              ...
              <div className="bg-gray-50 p-4 border border-gray-200 rounded text-sm print:bg-white print:border-black">
                <strong>💡 {reportTexts.recTitle}</strong>
                <ul className={`list-disc mt-2 space-y-1 ${lang === 'ar' ? 'mr-5' : 'ml-5'}`}>
                  {reportType === 'shortage' || reportData.some(i => Number(i.stock) < 50) ? (
                    <>
                      ...
                      <li>{lang === 'ar' ? 'يرجى توجيه أوامر توريد فورية.' : 'Immediate supply orders recommended.'}</li>
                    </>
                  ) : (
